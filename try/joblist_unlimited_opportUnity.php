<?php
    ...
    if(empty($list_jobid)) {
        $sql = "SELECT * FROM job WHERE job_status = 'Hiring' ORDER BY datetime_job_created DESC";
    } else {
        //eto yung responsible kaya di nakikita ng user yung job after nya mag apply
        $sql = "SELECT j.* FROM job j LEFT JOIN application_logs al ON j.jobid = al.jobid AND al.jobseeker_userid = ".$_SESSION['id']." WHERE (j.jobid NOT IN (" . $list_jobid_str . ") OR al.jobid IS NULL) AND (j.job_status = 'Hiring') ORDER BY datetime_job_created DESC";
    }
